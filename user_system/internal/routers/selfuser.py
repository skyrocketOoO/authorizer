...
from internal.usecases import selfuser as selfuser_usecase
...
def register(register_req: RegisterReq, db: Session = Depends(get_db)):
    try:
        selfuser_usecase.register(db, register_req.email, register_req.password, register_req.name)
...
def login(login_req: LoginReq, db: Session = Depends(get_db)):
    token = selfuser_usecase.login(db, login_req.email, login_req.password)
    if token:
        return {"token": token}
    else:
...
def get_current_user(token: annotated_bearer_middleware, db: Session = Depends(get_db)):
    ...
    user = selfuser_usecase.get_current_user(db, token)
    if user is None:
...
def update_user(update_user_req: UpdateUserReq, token: annotated_bearer_middleware, db: Session = Depends(get_db)):
    try:
        selfuser_usecase.update_user(db, token, update_user_req.name)
...
def delete_user(token: annotated_bearer_middleware, db: Session = Depends(get_db)):
    try:
        selfuser_usecase.delete_user(db, token)
    except error.UserNotFoundError as e:
        raise HTTPException(status_code=404, detail=e.message)
...
def change_password(change_password_req: ChangePasswordReq, token: annotated_bearer_middleware, db: Session = Depends(get_db)):
    try:
        selfuser_usecase.change_password(db, token, change_password_req.old_password, change_password_req.new_password)
    except error.UserNotFoundError as e:
        raise HTTPException(status_code=404, detail=e.message)
...
def is_token_valid(token: str = ""):
    # return True
    return selfuser_usecase.is_token_valid(token)
...
def forget_password(forget_password_req: ForgetPasswordReq, db: Session = Depends(get_db)):
    try:
        selfuser_usecase.forget_password(db, forget_password_req.email)
    except error.UserNotFoundError as e:
        raise HTTPException(status_code=404, detail=e.message)
...
def reset_password(reset_password_req: ResetPasswordReq, token: str = "", db: Session = Depends(get_db)):
    try:
        selfuser_usecase.reset_password(db, token, reset_password_req.new_password)
    except error.UserNotFoundError as e:
        raise HTTPException(status_code=404, detail=e.message)